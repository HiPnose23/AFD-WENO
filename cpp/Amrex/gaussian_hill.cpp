...
inline AMREX_GPU_DEVICE double vel_theta(double theta, double phi) {
    return u0 * std::sin(phi) * std::sin(alpha);
}
 
inline AMREX_GPU_DEVICE double vel_phi(double theta, double phi) {
    return u0 * (std::sin(theta) * std::cos(alpha)
               + std::cos(theta) * std::cos(phi) * std::sin(alpha));
}

// ==============================================================================
// EXACT SOLUTION (gaussian hill centered at (0,0)) 
// ==============================================================================
inline AMREX_GPU_DEVICE double exact_sol(double theta, double phi, double t) {
    const double x = std::sin(theta) * std::cos(phi);
    const double y = std::sin(theta) * std::sin(phi);
    const double z = std::cos(theta);
    return a0 * std::exp(-b0 * (x*x + (y+1.0)*(y+1.0) + z*z));
}
    ...
    amrex::Initialize(argc, argv);
    {
        ...
        for (amrex::MFIter mfi(u); mfi.isValid(); ++mfi) {
            ...
            amrex::ParallelFor(mfi.growntilebox(), [=] AMREX_GPU_DEVICE(int i, int j, int k) {
                arr(i,j,k) = exact_sol(get_theta(i), get_phi(j), 0.0);
            });
        }
        ...
        amrex::ReduceData<double, double> reduce_data(reduce_ops);
        ...
        for (amrex::MFIter mfi(u); mfi.isValid(); ++mfi) {
            ...
            reduce_ops.eval(mfi.validbox(), reduce_data, [=] AMREX_GPU_DEVICE(int i, int j, int k) -> ReduceTuple {
                double exact = exact_sol(get_theta(i), get_phi(j), t_final);
                double err   = std::abs(arr(i,j,k) - exact);
                return {err, std::abs(exact)};
            });
        }

        auto [num, den] = reduce_data.value();
        amrex::ParallelDescriptor::ReduceRealSum(num);
        amrex::ParallelDescriptor::ReduceRealSum(den);

        double l1_err = num / den;

        amrex::Print() << "\n--- Gaussian Hill Advection (5th Order) ---\n"
               << std::scientific << std::setprecision(8)
               << "l1 Error (unweighted): " << l1_err
